...
def degree_vs_DCi(degrees, DCis, origin, prefilter = True):
    """
    Plot relationship between degree and dominance candidate index (DCi) across treatments with exponential fit and quantile thresholds.
    ...
    if prefilter:
        plt.savefig(
            f"save_result/graphs/DCi_degree_change.svg",
            format="SVG",
            dpi=300,
            )
    else:
        plt.savefig(
            f"save_result/graphs/DCi_degree_change_noprefilter.svg",
            format="SVG",
            dpi=300,
            )
